<h1>{$Product['M_Name']} {$Product['Model_Name']}</h1>
            ...
            {foreach $Product['Picture'] as $item name=picture}
                <div class="column-images">
                    <img class="demo-images cursor"
                        src="/templates/default/images/products/{$item['DP_Device']}/{$item['DP_Image']}" style="width:100%"
                        onclick="currentSlide({$smarty.foreach.picture.iteration})">
                </div>
            {/foreach}
        </div>
    </div>
